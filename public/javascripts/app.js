var appConfig = { showFrameBox: false,
                  ...
                  grouping: {nearDistance: 150, viewingAngle: 85}, 
...
function loadData(step) {
  eraseCookie('config');
  if(step == 1) { 
    getData('data/200.json', function(data){
      eagleSenseData = data;
      loadData(step+1);
    });
  } else if(step == 2) {
...
function onFormInputUpdated(returnFunction){
  $('input.form-control, select.form-control').change(function(){
    ...
    else if($(this).attr('id') == 'person3BackwardMovementColor'){
      personConfig[2].movementColor.backward = hexToRgb( $(this).val());
      return;
    }
    else if($(this).attr('id') == 'person3ForwardMovementColor'){
      personConfig[2].movementColor.forward = hexToRgb( $(this).val());
      return;
    }
...
function loadForm(returnFunction){
  ...
  $('#person3BackwardMovementColor').val(rgbToHex(personConfig[2].movementColor.backward));
  $('#person3ForwardMovementColor').val(rgbToHex(personConfig[2].movementColor.forward));
